...
from learned_optimization import filesystem, profile
...
@profile.wrap()
def write_archive(task_name: str, hparam_set_name: str, data: Mapping[str, Any]):
...
def delete_archive(archive_name, task_name):
    p = os.path.join(get_baseline_archive_rootdir(), task_name, archive_name + ".npz")
    try:
        filesystem.remove(p)
        print("Deleted", p)
    except Exception as e:  # pylint: disable=broad-except
...
def get_save_dir(
    ...
    save_dir = os.path.join(
        get_root_baseline_dir() + save_dir_suffix,
        task_name,
        opt_name,
...
def load_baseline_results_from_dir(
    save_dir: str, output_type: str, threads: int = 0, clear_bad_files: bool = False
