...
from learned_optimization import filesystem, serialization_utils
...
class Mutate(abc.ABC):
    ...
    def update(
        self,
        state: MutateState,
        current_workers: Sequence[ActiveWorker],
    ...
    def get_worker_data(
        ...
        cache: MutableMapping[
            GenerationID,  # pylint: disable=unused-argument
...
class PopulationController:
    ...
    def maybe_get_worker_data(
        ...
        step: Optional[int],
        params: Optional[Any],
        meta_params: Optional[Any],
    ) -> Optional[ActiveWorker]:
        ...
        with self._lock:
            ...
            # to the current worker.
                ...
                logging.error(
                    "Potentially out of sync worker? Resetting worker to"
        ...
        with self._lock:
            ...
            if self._active_workers[worker_id].generation_id == generation_id:
                ...
                mut_active_workers = list(
                    self._active_workers
                )  # type: MutableSequence[ActiveWorker]
        ...
        if self._log_dir:
            ...
            if filesystem.exists(path):
                with filesystem.file_open(path, "rb") as f:
                    content = f.read()
                self._active_workers, self._cached, self._mutate_state = (
                    serialization_utils.safe_unpack(content)
                )
                return True
        return False
    ...
    def save_state(self):
        ...
        if self._log_dir:
            ...
            tmp_path = os.path.join(
                self._log_dir, f"population_tmp_{str(uuid.uuid4())}.state"
            )
