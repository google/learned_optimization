...
def eval_batch(params: Any, model_state: Any, key: PRNGKey, feats: Feats,
  ...
  l, _ = time_model.loss_fn.apply(params, model_state, key, feats, times)
  return l
...
def eval_many(params: Any,
  ...
  for _ in range(batches):
    ...
    losses.append(
        eval_batch(params, model_state, key1, batch["feats"], batch["time"]))
...
def _train_model(
    ...
    num_iters: int = 10000) -> Tuple[Tuple[Any, Any], Tuple[float, float]]:
  """Train a model and return weights and train/test loss."""
  batch = next(train_iter)

  key = jax.random.PRNGKey(0)

  p, s = time_model.loss_fn.init(key, batch["feats"], batch["time"])

  opt = opt_base.Adam(1e-5)
  ...
  def update(opt_state, key, feats, times):
    ...
    value_and_grad_fn = jax.value_and_grad(
        time_model.loss_fn.apply, has_aux=True)
  ...
  for i in range(num_iters):
    ...
    if (i < 100 and i % 10 == 0) or i % 100 == 0:
      p, s = opt.get_params_state(opt_state)
      train_loss.append(onp.asarray(eval_many(p, s, key, train_iter)))
      test_loss.append(onp.asarray(eval_many(p, s, key, test_iter)))
      print(i, train_loss[-1], test_loss[-1])

  return (p, s), (train_loss, test_loss)
...
def save_model(model: Any, sample_fn_name: str, hardware_name: str) -> str:
  dirname = time_model.get_timing_model_dir(sample_fn_name, hardware_name)
...
def train_and_save_timing_model(sample_fn_name: str = gin.REQUIRED,
  ...
  train_iter, test_iter = time_model_data.train_test_iterators(
      sample_fn_name, hardware_name, min_samples * 3, num_test=2000)
  model, (train_loseses, test_losses) = _train_model(train_iter, test_iter,
  ...
  path = save_model(model, sample_fn_name, hardware_name)
