...
from learned_optimization import (distributed, filesystem, profile, serialization_utils)
...
class TaskGroupChief(threading.Thread):
    ...
    def add_task_group(self, task_group: TaskGroup, tasks: Sequence[TaskConfig]):
        ...
        with self._lock:
            ...
            try:
                ...
                raise ValueError(
                    f"Must be able to hash the task_group!"  # pylint: disable=raise-missing-from
                    f" Got: {task_group}"
                )
    ...
    def get_work(self, worker_id: Any) -> Optional[EvalTask]:
        ...
        with self._lock:
            if self._worker_state[worker_id] is None:
                ...
                logging.info(
                    "Assigning worker%d to task_group%s id%d task %s",
                    worker_id,
                    task.task_group,
                    task.task_index,
                    task.task_content,
                ...
                logging.info(
                    "Resuming worker%d to task_group%s id%d for task %s",
                    worker_id,
                    task.task_group,
                    task.task_index,
                    task.task_content,
    ...
    def finish_work(self, worker_id: Any, result: Any):
        ...
        with self._lock:
            logging.info(
                "Worker %d finished work with result %s", worker_id, str(result)
            )
            task = self._worker_state[worker_id]
            ...
            self._tasks_in_taskgroup[task.task_group][task.task_index] = (
                dataclasses.replace(task_result, result=result)
            )

            self._schedule_save_state = True
            logging.info(
                "Worker %d is done with finish_work %s", worker_id, str(result)
            )
    ...
    def get_finished_task_group(
        self,
    ) -> Optional[Tuple[Any, Sequence[Any], Sequence[Any]]]:
